...
class PostgreSQLPersistentStore extends PersistentStore with PostgreSQLSchemaGenerator {
  ...
  PostgreSQLPersistentStore._from(PostgreSQLPersistentStore from)
      : isSSLConnection = from.isSSLConnection || from.sslMode.toSslMode() != SslMode.disable,
  ...
  factory PostgreSQLPersistentStore._transactionProxy(PostgreSQLPersistentStore parent, TxSession ctx) {
  ...
  static final Finalizer<Pool> _finalizer = Finalizer((connection) => connection.close());

  Pool? _databaseConnection;
  Completer<Pool>? _pendingConnectionCompleter;
      ...
      if (_pendingConnectionCompleter == null) {
        ...
        _connect()
          ...
          _pendingConnectionCompleter!.completeError(
            QueryException.transport("unable to connect to database", underlyingException: e),
          );
          _pendingConnectionCompleter = null;
        });
      }
  ...
  Future<T> transaction<T>(
    ...
    final Pool dbConnection = await getDatabaseConnection();
  ...
  Future<Schema> upgrade(Schema fromSchema, List<Migration> withMigrations, {bool temporary = false}) async {
    final Pool connection = await getDatabaseConnection();
    ...
    await connection.runTx((ctx) async {
      ...
      for (final migration in withMigrations) {
        ...
        final existingVersionRows = await ctx.execute(
          Sql.named("SELECT versionNumber, dateOfUpgrade FROM $versionTableName WHERE versionNumber >= @v:int4"),
  ...
  Future<dynamic> executeQuery(
      ...
      int timeoutInSeconds, {
        PersistentStoreQueryReturnType? returnType = PersistentStoreQueryReturnType.rows,
      }) async {
    final now = DateTime.now().toUtc();
    try {
      final dbConnection = await executionContext;
      ...
      return returnType == PersistentStoreQueryReturnType.rows ? results : results.affectedRows;
  ...
  QueryException<ServerException>? _interpretException(ServerException exception) {
  ...
  Future _createVersionTableIfNecessary(TxSession context, bool temporary) async {
  ...
  Future<Pool> _connect() async {
    ...
    final endpoints = [
      Endpoint(host: host!, database: databaseName!, port: port!, username: username, password: password),
    ];
