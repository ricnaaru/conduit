...
void main() {
  group("Alterations", () {
    ...
    test("Adding a table", () {
      ...
      expect(
        builder.schema.tables.firstWhere((st) => st.name == "foobar"),
        isNotNull,
      );
    ...
    test("Removing a table", () {
      ...
      expect(
        builder.schema.tables.firstWhereOrNull((st) => st.name == "_Container"),
        isNull,
      );
    });
    ...
    test("Adding column", () {
      builder.addColumn(
        "_DefaultItem",
        SchemaColumn("col1", ManagedPropertyType.integer),
      );
      builder.addColumn(
        "_defaultITEM",
        SchemaColumn("col2", ManagedPropertyType.integer),
      );
      expect(
        builder.schema
      ...
      try {
        builder.addColumn(
          "foobar",
          SchemaColumn("col3", ManagedPropertyType.integer),
        );
        expect(true, false);
      } on SchemaException catch (e) {
        expect(e.message, contains("does not exist"));
